  ...
  for stream in entry:
    if stream.name.startswith("__attach_version1.0_#"):
      try:
        ...
        log.error("Error processing attachment {} not found".format(str(e)))
        continue
  ...
  while i < len(stream):
    ...
    # Variable Length Properties.
      ...
      try:
        ...
        log.error("stream missing {}".format(streamname))
        continue
      ...
      try:
        ...
        log.error("Error while reading stream: {} not found".format(str(e)))
        continue
    else:
      # unrecognized type
      log.error("unhandled property type {}".format(hex(property_type)))
      continue
